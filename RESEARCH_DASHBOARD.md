...
> **更新时间（UTC）：2026-08-16 16:25 UTC**<br>
> **页面定位：**公开状态、量化门禁、负结果和下一步。主 README 保持简洁展示；`CURRENT_WORK.md` 给出双语边界。<br>
> **进程状态：**Stage 1 foundation 已验收；Stage 2 现在有一个有界 30 秒 final-AV extension/chunked formal r10 N=10 结果，60 秒仍未验收。<br>
> **核心边界：**30 秒 r10 是 practical/approximate Turbo + Sol-Attn extension/chunked 结果，不是 native long context、BF16 fidelity、人类语义/音频质量、产品就绪、公开对比或 SOTA 结论。
...
- **Stage 2 long-video objective：25/55。** 30 秒 final-AV extension/chunked formal r10 timing/structural 结果已验收；60 秒、人工语义/音频质量和产品就绪仍未验收。
- **长视频目标：1/2 bounded lanes。** 30 秒：已验收为 extension/chunked formal timing/structural lane；60 秒：未验收。
...
- **下一项：**60 秒 final-AV extension/chunked N=1 gate；真实 native long-context 研究保持单独标签，不用 extension/chunked 结果冒充。
...
| Stage 2 | M5 60 秒 final-AV extension/chunked lane | 15% | 单 A6000 产生并验收 60s 输出，AV/资源/耗时边界通过 | 0% | `benchmark_contract/v1/lane-manifests/final-av-60s-1344x768-24fps-v1.json` | 未验收 |
| Stage 2 | M6 长视频泛化、人工质量与最终产品边界 | 10% | 30/60s 通过后，多 prompt/seed、N≥3/N≥10、人工质量与 claim 审查 | 0% | 待生成 | 未开始 |
|  | **合计** | **100%** |  | **70%** | 快照：2026-08-16 16:25 UTC | Stage 1 = 45/45；Stage 2 = 25/55 |
...
| 60s 长视频验收 | N/A（未测） | N/A | N/A | 0/1 accepted target | 需先执行 60s N=1 gate | 未验收 |
...
flowchart LR
    ...
    B --> C[当前发布：更新 public reports + audit + Reviewer-bound push]:::current
    C --> D[下一实验：60s final-AV extension/chunked N=1]:::pending
    N[原生长上下文研究]:::blocked -.单独研究，不能由 extension 冒充.-> D
    H[人工语义/音频质量 gate]:::pending -.产品质量前必须单独通过.-> D
