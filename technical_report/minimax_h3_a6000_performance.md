...
- Sol-Attn pair-value-halves SM86 captured replay: evidence `sol_engine_port/sol_attn_pair_value_halves_captured_replay_20260813T145731Z/summary.json`, `decision.json`, and `validation.json`; the prior standalone harness remains at `sol_engine_port/sol_attn_pair_value_halves_20260813T110953Z/summary.json` and `harness.json`. The captured replay has model_load=False and docker_used_for_run=False, used GPU UUID `GPU-b3425477-0877-24de-c5b8-1549ab47cd4b` as exactly one visible GPU under Argus lease supervision, and reused captured r8 H3 metadata recording `sparse_calls=192`, `fallback_calls=0`, and materialized_copy=192 / 105344139264 bytes. The default-off `MINIMAX_H3_A6000_SOL_ATTN_PAIR_VALUE_HALVES` candidate preserved stride-aware V, full-prefix-block skip, dense-prefix overwrite, tau/routing, and exact-block order while sharing one Q/K route/probability stream for two BV64 value halves. Correctness/telemetry: captured layout agreement and metadata derivation passed, `max_abs_valid=0`, candidate prefix/tail equal to current, padding rows zero, and both current/candidate replay lanes had zero fallback and zero unintended materialization. Captured-metadata/model-free phase medians on B=1,T_total=38272,T_valid=38247,H=56,D=128,prefix=951: candidate total 144.65228271484375 ms vs current prefix-skip 175.2156219482422 ms; candidate forward pointer 129.85037231445312 ms vs current 158.160888671875 ms. Boundary: captured-metadata/non-Docker/kernel/model-free evidence only; not H3 E2E, not real-chain speedup, not long-video, not BF16 fidelity, not normal-PC/product evidence, not public-comparison evidence, and not SOTA.
...
- `sol_engine_port/sol_attn_pair_value_halves_20260813T110953Z/harness.json`
- `sol_engine_port/sol_attn_pair_value_halves_20260813T110953Z/summary.json`
- `sol_engine_port/sol_attn_pair_value_halves_captured_replay_20260813T145731Z/decision.json`
- `sol_engine_port/sol_attn_pair_value_halves_captured_replay_20260813T145731Z/summary.json`
- `sol_engine_port/sol_attn_pair_value_halves_captured_replay_20260813T145731Z/validation.json`
