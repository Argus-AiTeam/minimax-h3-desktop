...
- Pair-value-halves SM86 captured replay: `retained_model_free_pair_value_halves_kernel_candidate`; evidence `sol_engine_port/sol_attn_pair_value_halves_captured_replay_20260813T145731Z/summary.json`, `decision.json`, and `validation.json`; prior standalone harness evidence remains at `sol_engine_port/sol_attn_pair_value_halves_20260813T110953Z/summary.json` and `harness.json`; model_load=False; docker_used_for_run=False; GPU UUID `GPU-b3425477-0877-24de-c5b8-1549ab47cd4b`; exactly one visible GPU under Argus lease supervision. Captured source checks reused r8 metadata with sparse_calls=192, fallback_calls=0, and materialized copies=192 / 105344139264 bytes. Correctness/telemetry: captured layout and metadata derivation passed, `max_abs_valid=0`, prefix/tail equal to current, padding rows zero, zero replay fallback, and zero unintended materialization. Captured-metadata phase medians: candidate total=144.65228271484375 ms vs current prefix-skip=175.2156219482422 ms; candidate forward pointer=129.85037231445312 ms vs current=158.160888671875 ms. Boundary: captured-metadata/non-Docker/kernel/model-free evidence only; not H3 end-to-end, not long-video, not BF16 fidelity, not real-chain/product speedup, not normal-PC evidence, not public-comparison evidence, and not SOTA.
