...
# External build-only r6 overlay image generator. It records source, patch-file,
...
DOCKERFILE=ports/minimax_h3_a6000/integration/r6/Dockerfile
PATCH=ports/minimax_h3_a6000/patches/vllm_omni_h3_a6000_opt_in.patch
HELPER=ports/minimax_h3_a6000/integration/r6/dual_install_patch_files.py

cd "$ROOT"

if [[ ! -f "$DOCKERFILE" ]]; then
  echo "missing r6 Dockerfile; run from repository root or set ROOT" >&2
  exit 2
fi
if [[ ! -f "$PATCH" ]]; then
...
if [[ ! -f "$HELPER" ]]; then
  echo "missing r6 patch install helper: $HELPER" >&2
  exit 2
fi

mkdir -p "$EVIDENCE_DIR"
python3 "$HELPER" --patch "$PATCH" --list-patch-files > "$EVIDENCE_DIR/r6_patch_changed_files.txt"
python3 - "$EVIDENCE_DIR/r6_source_hashes.sha256" "$EVIDENCE_DIR/r6_source_hashes.json" <<'PY'
...
inputs = [
    pathlib.Path("ports/minimax_h3_a6000/integration/r6/Dockerfile"),
    pathlib.Path("ports/minimax_h3_a6000/integration/r6/build_r6_overlay_image.sh"),
    pathlib.Path("ports/minimax_h3_a6000/integration/r6/dual_install_patch_files.py"),
...
out_json.write_text(
    json.dumps(
        {
            "schema_version": "minimax_h3_a6000_r6_source_hashes_v1",
            "records": records,
        },
        indent=2,
        sort_keys=True,
...
printf 'base_image=%s\ntag=%s\ndockerfile=%s\npatch=%s\npatch_sha256=%s\nnetwork=none\ngpu_flags=none\npull=false\n' \
  "$BASE_IMAGE" "$TAG" "$DOCKERFILE" "$PATCH" "$patch_sha" > "$EVIDENCE_DIR/r6_build_params.env"
...
DOCKER_BUILDKIT=${DOCKER_BUILDKIT:-1} docker build --pull=false --network=none \
  ...
  -f ports/minimax_h3_a6000/integration/r6/Dockerfile \
  -t "$TAG" \
  --iidfile "$EVIDENCE_DIR/r6_image_iid.txt" \
  .
docker image inspect "$TAG" > "$EVIDENCE_DIR/r6_image_inspect.json"
printf 'External diagnostic image identity: image=%s iid_file=%s inspect=%s\n' \
  "$TAG" "$EVIDENCE_DIR/r6_image_iid.txt" "$EVIDENCE_DIR/r6_image_inspect.json" \
  > "$EVIDENCE_DIR/r6_image_identity_summary.txt"
