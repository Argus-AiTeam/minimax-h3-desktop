...
OUT_DIR=${OUT_DIR:-$ROOT/technical_report/evidence/minimax_h3_desktop/sol_engine_port/r4_integration_$(date -u +%Y%m%dT%H%M%SZ)}
PORT=${PORT:-8000}
...
for op, label in required.items():
    ...
    copy_summary[op] = {
        ...
        'materialize_copy_by_tensor': item.get('materialize_copy_by_tensor', {}),
    }
for op in ('indexed_modulate_bf16', 'indexed_gate_bf16'):
    item = ops[op]
    strategies = strategy_summary[op]
    assert strategies.get('stride_aware', 0) > 0, f'{op} did not use stride-aware candidates: {strategies}'
    assert 'materialize' in strategies, f'{op} missing materialize strategy counter: {strategies}'
    assert item.get('tensor_layout_samples'), f'{op} missing tensor_layout_samples telemetry: {item}'
    assert item.get('tensor_layout_summary'), f'{op} missing tensor_layout_summary telemetry: {item}'
    assert 'materialize_copy_by_tensor' in item, f'{op} missing materialize copy source telemetry: {item}'
status = {
    'status': 'pass',
