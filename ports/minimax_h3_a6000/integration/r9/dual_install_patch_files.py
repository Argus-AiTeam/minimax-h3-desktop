...
"""Install the r9 vLLM-Omni patch outputs into both image import surfaces.

The r9 Dockerfile applies the current MiniMax-H3 A6000 opt-in patch to a
throwaway copy of `/app/vllm-omni`, then invokes this helper.  The helper audits
that the patch-changed file list is exactly the locked r9 list, installs every
...
def extract_patch_changed_files(patch_text: str) -> list[str]:
    ...
    if sorted(changed) != sorted(EXPECTED_PATCH_CHANGED_FILES):
        raise ValueError(
            "r9 patch changed-file audit mismatch: "
...
def install_changed_files(
    *,
    ...
    hash_json_path.write_text(
        json.dumps(
            {
                "schema_version": "minimax_h3_a6000_r9_patched_source_hashes_v1",
