...
# External single-A6000 r9 adaptive-routing matched N=1 gate.
...
OUT_DIR=${OUT_DIR:-$ROOT/technical_report/evidence/minimax_h3_desktop/sol_engine_port/sol_attn_h3_adaptive_routing_r9_n1_$(date -u +%Y%m%dT%H%M%SZ)}
PORT=${PORT:-8000}
...
usage() {
  cat <<'EOF_USAGE'
Usage: run_sol_attn_h3_adaptive_routing_n1.sh [--dry-run|--execute]

Dry-run is the default. Non-dry execution requires
ARGUS_ALLOW_A6000_SOL_ATTN_ADAPTIVE_ROUTING_N1=1, a readable fresh r9 image,
fresh single-A6000 hygiene, and fixed 5-step H3 workload variables.

Modes:
  1. current_retained_a: retained r9 Sol-Attn lane, tau=1.0/diag, adaptive off;
  2. current_retained_b: same retained lane for decoded-AV/attention digest stability;
  3. adaptive_tau2_diag: same lane with adaptive routing on, tau=2.0, thresh_type=diag.
All modes keep cache off, stride-aware V on, full-prefix-block skip on,
pair-value-halves/static/bitmask/exact-prefix off, dense-prefix overwrite preserved,
one excluded warmup, one measured request, and bounded output-digest diagnostics.
EOF_USAGE
}
...
principal_variable=MINIMAX_H3_A6000_SOL_ATTN_ADAPTIVE_ROUTING=1 with tau=2.0 thresh_type=diag vs retained tau=1.0 diag
policy=cache off; stride-aware V on; full-prefix-block skip on; dense-prefix overwrite preserved; pair-value-halves/static-prefix/bitmask/exact-prefix off; dense_first_steps=0; dense_first_layers=2
current_modes=current_retained_a,current_retained_b
candidate_mode=adaptive_tau2_diag
classification=matched_N1_practical_approx_gate_not_formal_speedup_claim
Would execute only with:
  ARGUS_ALLOW_A6000_SOL_ATTN_ADAPTIVE_ROUTING_N1=1 bash ports/minimax_h3_a6000/integration/run_sol_attn_h3_adaptive_routing_n1.sh --execute
EOF_PLAN
}
...
if [[ "${ARGUS_ALLOW_A6000_SOL_ATTN_ADAPTIVE_ROUTING_N1:-0}" != "1" ]]; then
  echo "ERROR: refusing non-dry Sol-Attn A6000 adaptive-routing gate without ARGUS_ALLOW_A6000_SOL_ATTN_ADAPTIVE_ROUTING_N1=1" >&2
  exit 11
fi
...
comparison=current_retained_a_vs_current_retained_b_then_adaptive_tau2_diag
lane=matched_n1_5step_sol_attn_opt_in_practical_approx_adaptive_routing
principal_variable=MINIMAX_H3_A6000_SOL_ATTN_ADAPTIVE_ROUTING_tau2_diag_vs_retained_tau1_diag
sol_attn_cache=off
sol_attn_stride_aware_v=on
sol_attn_skip_full_prefix_blocks=on
sol_attn_adaptive_routing=current_modes_off_candidate_on
sol_attn_candidate_tau=2.0
sol_attn_candidate_thresh_type=diag
...
write_adaptive_runtime_failure_decision() {
  local current_a_rc="$1" current_b_rc="$2" candidate_rc="$3"
  "$HOST_PYTHON" - <<'PY' "$OUT_DIR" "$current_a_rc" "$current_b_rc" "$candidate_rc" "$VLLM_OMNI_INIT_TIMEOUT_S" "$VLLM_OMNI_STAGE_INIT_TIMEOUT_S" "$SERVER_READY_TIMEOUT_S" "$REQUEST_TIMEOUT_S"
import json, pathlib, sys, time
root=pathlib.Path(sys.argv[1]); rcs={'current_retained_a':int(sys.argv[2]),'current_retained_b':int(sys.argv[3]),'adaptive_tau2_diag':int(sys.argv[4])}
timeouts={'vllm_omni_init_timeout_s':int(sys.argv[5]),'vllm_omni_stage_init_timeout_s':int(sys.argv[6]),'server_ready_timeout_s':int(sys.argv[7]),'request_timeout_s':int(sys.argv[8])}
first=next((m for m,rc in rcs.items() if rc), None)
decision={'schema_version':'minimax_h3_a6000_sol_attn_adaptive_routing_n1_runtime_failure_v1','classification':'blocked','decision':'blocked_runtime_failure','reason':f'{first}_failed' if first else 'unknown_failure','return_codes':rcs,'timeout_values':timeouts,'promote_to_matched_n3':False,'promote_to_formal_speedup':False,'not_speedup_claim':True,'no_product_speedup_claim':True,'timestamp_unix':time.time(),'lane':'matched_n1_5step_sol_attn_opt_in_adaptive_routing_practical_approx','workload':{'width':1344,'height':768,'frames':124,'fps':24,'duration_s':5.166667,'steps':5,'seed':0},'principal_variable':'adaptive tau=2.0 diag versus retained tau=1.0 diag','claim_boundary':'Startup/runtime failure artifact only; no product speedup, BF16-fidelity, long-video, quality-equivalence, or promotion claim.'}
(root/'decision.json').write_text(json.dumps(decision,indent=2,sort_keys=True)+'\n',encoding='utf-8')
print(root/'decision.json')
PY
}
...
run_one current_retained_a   "${common_sol_env[@]}"   -e MINIMAX_H3_A6000_SOL_ATTN_ADAPTIVE_ROUTING=0   -e MINIMAX_H3_A6000_SOL_ATTN_TAU=1.0   -e MINIMAX_H3_A6000_SOL_ATTN_THRESH_TYPE=diag   -e MINIMAX_H3_A6000_SOL_ATTN_TELEMETRY_ARM_FILE=/evidence/current_retained_a/measure.arm   -e MINIMAX_H3_A6000_SOL_ATTN_TELEMETRY_JSON=/evidence/current_retained_a/sol_attn_telemetry
current_a_rc=$?
current_b_rc=0
candidate_rc=0
if [[ "$current_a_rc" -eq 0 ]]; then
  run_one current_retained_b     "${common_sol_env[@]}"     -e MINIMAX_H3_A6000_SOL_ATTN_ADAPTIVE_ROUTING=0     -e MINIMAX_H3_A6000_SOL_ATTN_TAU=1.0     -e MINIMAX_H3_A6000_SOL_ATTN_THRESH_TYPE=diag     -e MINIMAX_H3_A6000_SOL_ATTN_TELEMETRY_ARM_FILE=/evidence/current_retained_b/measure.arm     -e MINIMAX_H3_A6000_SOL_ATTN_TELEMETRY_JSON=/evidence/current_retained_b/sol_attn_telemetry
  current_b_rc=$?
fi
if [[ "$current_a_rc" -eq 0 && "$current_b_rc" -eq 0 ]]; then
  run_one adaptive_tau2_diag     "${common_sol_env[@]}"     -e MINIMAX_H3_A6000_SOL_ATTN_ADAPTIVE_ROUTING=1     -e MINIMAX_H3_A6000_SOL_ATTN_TAU=2.0     -e MINIMAX_H3_A6000_SOL_ATTN_THRESH_TYPE=diag     -e MINIMAX_H3_A6000_SOL_ATTN_TELEMETRY_ARM_FILE=/evidence/adaptive_tau2_diag/measure.arm     -e MINIMAX_H3_A6000_SOL_ATTN_TELEMETRY_JSON=/evidence/adaptive_tau2_diag/sol_attn_telemetry
  candidate_rc=$?
fi
set -e
...
if [[ "$combined_rc" -eq 0 ]]; then combined_rc=$current_b_rc; fi
if [[ "$combined_rc" -eq 0 ]]; then combined_rc=$candidate_rc; fi
...
if [[ "$combined_rc" -ne 0 ]]; then
  write_adaptive_runtime_failure_decision "$current_a_rc" "$current_b_rc" "$candidate_rc"
  echo "ERROR: adaptive-routing lane failed; decision in $OUT_DIR/decision.json" >&2
  exit "$combined_rc"
fi

"$HOST_PYTHON" "$SCRIPT_DIR/finalize_sol_attn_adaptive_routing_diagnostic.py" "$OUT_DIR"
