...
Status: **default-off standalone Triton exact-kernel candidates, a real SM86 pointer-path Sol-Attn candidate, terminal r8 H3 Sol-Attn 5-step sparse-execution diagnostic evidence, and a nonterminal r8 formal N>=10 inspection**. This directory does not modify the locked vLLM-Omni worktree, does not load MiniMax-H3 weights during CPU/static work, and keeps all runtime hooks default-off/fail-closed. The selected H3 Sol-Attn 5-step diagnostic run is terminal (`complete`, exit code `0`) and is attributed by readable r8 workload/version-label provenance (image tag/version/title and `H3_A6000_SOL_ATTN` workload), not opaque image/output identifier equality. The r8 diagnostic recorded dense and opt-in HTTP 200 plus structural AV evidence, `sparse_candidate_calls=192`, `sparse_calls=192`, `fallback_calls=0`, 192 density samples, and materialized-copy/resource telemetry; therefore the old r7 `fail_closed_missing_metadata` blocker is cleared for the fixed 5-step metadata gate. This is only a 5-step sparse-execution metadata-plumbing candidate pass, not a speedup, N10, BF16 fidelity, release, or quality-equivalence claim. The r8 N=3 matched-workload route gate is terminal at `technical_report/evidence/minimax_h3_desktop/sol_engine_port/sol_attn_h3_matched_retest_r8_n3_20260812T013544Z/decision.json`: all three dense/opt-in pairs completed with HTTP 200, structural AV, sparse_calls=192/fallback_calls=0 per opt-in pair, median HTTP-time improvement 14.782455716069165% over a >3.0% route threshold, and resource envelope within the predeclared gate. The later formal N>=10 run directory `technical_report/evidence/minimax_h3_desktop/sol_engine_port/sol_attn_h3_formal_n10_r8_n10_20260812T031757Z` is currently classified only as `incomplete_formal_n10_running_no_terminal_decision`: fresh CPU live recheck `technical_report/evidence/minimax_h3_desktop/delivery/formal_n10_live_recheck_20260812T035433Z/summary.json` found supervisor status `running`, 3 started pair directories, 2 completed pairs with `sparse_runtime_valid` diagnostics, pair03 without terminal artifacts, and no `formal_n10_decision.json` or formal `RUN_REPORT.md`. Therefore no formal Sol-Attn speedup, BF16 fidelity, release, or quality-equivalence claim is accepted from it.
